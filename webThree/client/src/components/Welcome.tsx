...
import { ContextInfo, TransactionContext } from "../context/TransactionContext";
import { Loader } from "./";
...
const Welcome = () => {
    const { connectWallet, currentAccount, formData, handleChange, sendTransaction } =
        useContext<ContextInfo>(TransactionContext);

    const handleSubmit = (e: React.MouseEvent<HTMLButtonElement>) => {
        const { addressTo, amount, keyword, message } = formData;
        e.preventDefault();
        console.log("formData :>> ", formData);
        if (_.some([addressTo, amount, keyword, message], _.negate(_.isEmpty))) {
            return;
        }
                ...
                <div className='flex flex-col flex-1 items-center justify-start w-full mf:mt-0 mt-10'>
                    <div className='p-3 justify-end items-start flex-col rounded-xl h-40 sm:w-72 w-full my-5 eth-card white-glassmorpism'>
                        <div className='flex justify-between flex-col w-full h-full'>
                            ...
                            <div>
                                <p className='text-white font-light text-sm'>0x</p>
